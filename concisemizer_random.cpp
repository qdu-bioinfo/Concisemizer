// concisemizer_random sampling
// compile：g++ -std=c++11 concisemizer_random.cpp -o concisemizer_random
// operation：./concisemizer_random -n XXX.fna -k xxx -w xxx
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 7)
    {
        std::cout << "Error: No arguments provided." << std::endl
                  ...
                  << "./concisemizer_random -n XXX.fna -k xxx -w xxx" << std::endl
                  << std::endl;
        return 0;
    }
    // Obtain option
    ...
    // H-hits
    std::cout << seed_max_hit / (double)seed_sum << std::endl;
                ...
                for (unsigned long long int i = 0; i < line_n; i++)
                {
                    ...
                    if (ATUCG[genome_line[i]] != 4)
                    {
                        ...
                        if (dq_min_bj == 0) // What is deleted is the minimum value. A new minimum value must be found and saved.
                        {
                            ...
                            auto it = dq_minimizer.begin();
                            minimizer_mini = *it;
                            minimizer_minihash = hash64(*it, mask);
                            dq_min_bj = 1;
                            int i = 1;
                            while (++it != dq_minimizer.end())
                            {
                                i++;
                                if (hash64(*it, mask) <= minimizer_minihash) // Same as min select new.
                                {
                                    minimizer_mini = *it;
                                    minimizer_minihash = hash64(*it, mask);
                                    dq_min_bj = i; // Point to the new minimum value in dp
    ...
    // DR-Score
    // std::cout << E_hits2 / (double)seed_sum << std::endl;
    std::cout << (seed_sum * k_mer_win / (double)s_kmer_sum) * E_hits2 / (double)seed_sum << std::endl;
    return 0;
}
