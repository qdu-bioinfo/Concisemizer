// concisemizer_lexicon sampling
// compile：g++ -std=c++11 concisemizer_lexicon.cpp -o concisemizer_lexicon
// operation：./concisemizer_lexicon -n XXX.fna -k xxx -w xxx
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 7)
    {
        std::cout << "Error: No arguments provided." << std::endl
                  ...
                  << "./concisemizer_lexicon -n XXX.fna -k xxx -w xxx" << std::endl
                  << std::endl;
        return 0;
    }
    // Obtain option
    ...
    int k_mer_yes_no = 0;          // Is the second seed in the triplet
    unsigned long long int x1 = 0; // The first seed of the triplet has already been determined
    unsigned long long int suoyin1 = 0;
    unsigned long long int x2 = 0; // For the second seed of the triplet, based on the third one, see whether to keep or delete this 2
    unsigned long long int suoyin2 = 0;
            ...
            for (unsigned long long int i = 0; i < line_n; i++)
            {
                ...
                if (ATUCG[genome_line[i]] != 4)
                {
                    ...
                    if (dq_min_bj == 0) // What is deleted is the minimum value. A new minimum value must be found and saved.
                    {
                        ...
                        {
                            i++;
                            if ((*it).first <= minimizer_mini.first) // Same as min select new.
                            {
                                minimizer_mini = *it;
                        ...
                        if (k_mer_yes_no == 0)
                        {
                            x2 = minimizer_mini.first;
                            suoyin2 = minimizer_mini.second;
                            k_mer_yes_no = 1;
                        }
                        else
                        {
                            if ((minimizer_mini.second - suoyin1) <= k_mer_win)
                            {
                                x2 = minimizer_mini.first;
                                suoyin2 = minimizer_mini.second;
                            }
                            else
                            {
                                x1 = x2;
                                suoyin1 = suoyin2;
                                save_k_mer.push_back(make_pair(hash64(x2, mask), suoyin2));
                                x2 = minimizer_mini.first;
                                suoyin2 = minimizer_mini.second;
                            }
                        }
                    ...
                    else // What is deleted is not the minimum value. If the newly inserted element is min, it will be saved.
                    {
                        ...
                        if (x3 <= minimizer_mini.first) // Same as min select new.
                        {
                ...
                else // !atcgu Base, Find the new kmer deque.
                {
                    ...
                    {
                        k_mer_yes_no = 0;
                        x1 = minimizer_mini.first;
                        suoyin1 = minimizer_mini.second;
    ...
    std::cout << save_k_mer.size() / (double)s_kmer_sum << std::endl;
    std::cout << save_k_mer.size() * k_mer_win / (double)s_kmer_sum << std::endl;

    // H-hits
    unsigned long long int seed_max_hit = 0;
    ...
    {
        seed_max_hit = seed_max_hit + pair.second * pair.second;
    }

    // H-hits
    std::cout << seed_max_hit / (double)save_k_mer.size() << std::endl;
    // DR-Score
    unsigned long long int E_hits2 = 0; // Extended E_hits

    {
        ...
        unsigned long long int minimizer_mini; // The value of mini.
        minimizer_mini = numeric_limits<unsigned long long int>::max();
        int dq_min_bj = 1; // The position of the minimum element in the deque, random index access flag
        ...
        vector<pair<unsigned long long int, unsigned long long int>> save_k_mer; // Save the found minimizer and its location information.
        deque<unsigned long long int> dq_minimizer;                              // Insert at the end of the queue, delete at the beginning of the queue, and traverse.
                ...
                for (unsigned long long int i = 0; i < line_n; i++)
                {
                    ...
                    if (ATUCG[genome_line[i]] != 4)
                    {
                        ...
                        if (dq_min_bj == 0) // What is deleted is the minimum value. A new minimum value must be found and saved.
                        {
                            ...
                            {
                                i++;
                                if (*it <= minimizer_mini) // Same as min select new.
                                {
                                    minimizer_mini = *it;
                                    dq_min_bj = i; // Point to the new minimum value in dp
                                }
                            }
                            if (map_minimizer_sum.count(hash64(minimizer_mini, mask)))
                            {
                                E_hits2 = E_hits2 + map_minimizer_sum[hash64(minimizer_mini, mask)];
                            }
                        }
                        else // What is deleted is not the minimum value. If the newly inserted element is min, it will be saved.
                        {
                            ...
                            // If they are the same, do nothing
                            // If they are different, save
                            if (x3 <= minimizer_mini) // Same as min select new.
                            {
                                minimizer_mini = x3;
                                dq_min_bj = k_mer_win; // Point to the new minimum value in dp
                                if (map_minimizer_sum.count(hash64(minimizer_mini, mask)))
    ...
    // DR-Score
    // std::cout << E_hits2 / (double)save_k_mer.size() << std::endl;
    std::cout << (save_k_mer.size() * k_mer_win / (double)s_kmer_sum) * E_hits2 / (double)save_k_mer.size() << std::endl;
    return 0;
}
